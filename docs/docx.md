...
#### Using a placeholder image in the document
...
#### Using just the helper call to generate an image from scratch
1. Add a helper call `{{docxImage src=myDataURIForImage}}` in the document
2. Run the template with `myDataURIForImage` prop in the input data and you should see the image replaced in the output.
...
- `src` (`string|function`) -> specifies the base64 dataURI string representation of the image to load or an url from which to fetch the image. When set to a function, this function will be used to load the content of the image, allowing to fetch the image using a custom implementation.
...
- `fallbackSrc|function` (`string`) -> specifies the base64 dataURI string representation of the image to load or an url from which to fetch the image to use as fallback in case the `src` image fails to load. Ex: `{{docxImage src=src fallbackSrc=fallbackSrc}}`. When set to a function, this function will be used to load the content of the image, allowing to fetch the image using a custom implementation.

Options only available when using a placeholder image in the document:
- `usePlaceholderSize` (`boolean`) -> (ONLY available when using a placeholder image in the document) when true the dimensions of the image will be set to the same dimensions than the placeholder image defined on the docx file. Ex: `{{docxImage src=src usePlaceholderSize=true}}`
...
#### Loading images using a custom loader
You can set `src`, `fallbackSrc` to use a loader function. This will allow to have more control about how an image is fetched, letting implement custom logic to load images.

First the `docxImage` call needs to be updated to use a helper for the src: `{{docxImage src=(imageLoader imageInput)}}`
then you need to add the function in your helpers. The helper function needs to return another function which will run when the time to fetch the image comes.
The child function should return an object with the information of the image. it should have a `type` (to indicate if the image is `png`, `jpg`, etc) and a `stream` (a readable stream that represents the content of the image).

A basic loader function will look like this:
...
function imageLoader(src) {
    ...
    return async function loader () {
        ...
        // a custom http request library, load from fs or from S3, etc. the user is also
        // in control of the input, you can pass to the helper a path to a local file if it is
        // what the loader needs for its logic.
...
The loader function should return an object with the information of the image. it should have a `type` (to indicate if the image is `png`, `jpg`, etc) and a `stream` (a readable stream that represents the content of the image).

A basic helper for the `imageLoader` option will look like this:

```js
function imageLoader() {
    // a loader should always return a stream and type (extension or content type for image)
    return async function loader (src) {
        // you will receive the src of the image as the input of the loader, you can use this value
        // to fetch your image using custom logic
        const fs = require('fs')
